    ...
    int grid_width)
{
    ...
    double xNew = (xL + xR + xB + xT - g + alpha * b) / 3.0;
    ...
    int grid_height)
{
    ...
    jacobi_boundary_pressure_setVal(frame_out, b_in, i, j, xL, xR, xB, xT, g, alpha, grid_width);
    frame_out[g_index] = frame_out[getIndex(i, j, grid_width)] - g;  /* enforces dp/dx ~ w_0 */
}
    ...
    const int grid_height)
{
    ...
    if (k > 0 && k < (grid_height - 1)) {
        j = k;
        jB = j - 1;
        jT = (k < (grid_height - 1)) ? j + 1 : -1;
        /* go along i=1 */
        i = 1;
        iR = i + 1;
        iL = -1;
        jacobi_boundary_pressure_doSide(frame_out, frame_in, Wx_in, b_in, i, iL, iR, j, jT, jB, delta_x, grid_width, grid_height);
        /* go along i=i_max-1 */
        i = grid_width - 2;
        iR = -1;
        iL = i - 1;
    ...
    //if (k > 0 && k < (grid_width - 1)) {
        i = k;
        iL = i - 1;
        iR = (k < (grid_width - 1)) ? i + 1 : -1;
        /* go along j=1 */
        j = 1;
        jT = j + 1;
        jB = -1;
        jacobi_boundary_pressure_doSide(frame_out, frame_in, Wy_in, b_in, i, iL, iR, j, jT, jB, delta_x, grid_width, grid_height);
        /* go along j=j_max-1 */
        j = grid_height - 2;
        jT = -1;
        jB = j - 1;
